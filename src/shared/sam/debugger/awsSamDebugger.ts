    ...
    private async makeAndInvokeConfig(
        ...
        try {
            ...
            if (err instanceof SamLaunchRequestError) {
                await err.showNotification()
            } else if (err instanceof ToolkitError) {
                await new SamLaunchRequestError(err.message, { ...err }).showNotification()
            } else {
                await SamLaunchRequestError.chain(err, 'Failed to run launch configuration').showNotification()
            }
        }
    }
    ...
    public async makeConfig(
        ...
        if (!hasLaunchJson) {
            ...
            if (!rv.isValid) {
                ...
                await vscode.window.showInformationMessage(rv.message)
            }
